...
const AuthForm = () => {
  ...
  const authUserHandler = event => {
    ...
    fetch(url, {
      ...
      // console.log(data);
      const milliSeconds = Number(data.expiresIn) * 1000;
      const futureDate = new Date((new Date().getTime()) + milliSeconds).toISOString();
      authContext.login(data.idToken, futureDate);
      history.replace("/");
    }).catch(error => {
      alert(error);
    })
